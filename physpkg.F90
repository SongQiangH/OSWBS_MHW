...
subroutine tphysbc (ztodt,               &
    ...
    !*** CJX MOD START: Define Variables
    ...
    !*** CJX MOD START: Convective Heating Force Modification 
calday = get_curr_calday();
 ...
 do i = 1,ncol 
     ...
     if (stat_lat >= latS .and. &
     stat_lat <= latN .and. &
     stat_lon >= lonW .and. &
     stat_lon <= lonE)  then
         irate = (1 - abs(stat_lat-latC)/abs(latN-latC))*(1 -abs(stat_lon-lonC)/abs(lonE-lonC))
         if(irate >= 0) then
             ! Inject the heat force to MAM
             do k = 1,pver
                 if (j == 7 .and. ptend%s(i,k)>0) then
                     ptend%s(i,k) = ptend%s(i,k)*(3+irate)
                 end if
             end do
         end if    
     end if
    ...
    !*** CJX MOD END: Convective Heating Force Modification 
